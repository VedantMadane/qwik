...
import { _dumpState } from '@qwik.dev/core/internal';
...
test.describe('router ssg snapshot', () => {
  ...
  test('build output matches the checked-in normalized html and state dump', async () => {
    ...
    await expect(normalizedHtml).toEqual(await readFile(expectedHtmlPath, 'utf-8'));
    await expect(normalizedState).toEqual(await readFile(expectedStatePath, 'utf-8'));
  });
});
