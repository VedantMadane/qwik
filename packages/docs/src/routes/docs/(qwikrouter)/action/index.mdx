...
<Note>Since actions are not executed during rendering, they can have side effects such as writing to a database, or sending an email. An action only runs when called explicitly.</Note>
...
<Note>The server re-renders the whole page and re-executes everything, so if you have any [routeLoader$](/docs/route-loader/) they will be executed too.</Note>
...
Failures are stored in the `action.error` property. When the action fails, `action.error` will contain the error data. Failure messages can be found in the `fieldErrors` object according to properties defined in your Zod schema.
...
Thanks to Typescript type discrimination, you can use the `action.error` property to discriminate between success and failure.
