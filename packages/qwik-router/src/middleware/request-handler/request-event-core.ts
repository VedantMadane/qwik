...
import { getRouteLoaderData, getRouteLoaderValues } from '../../runtime/src/route-loaders';
  ...
  const requestEv: RequestEventInternal = {
    ...
    resolveValue: (async (loaderOrAction: LoaderInternal | ActionInternal) => {
      if (loaderOrAction.__brand === 'server_loader') {
        ...
        // Run the loader on-demand (for cross-loader dependency resolution)
        const value = await getRouteLoaderData(
          loaderOrAction.__qrl,
          loaderOrAction.__validators,
          requestEv
        );
        loaderValues[loaderOrAction.__id] = value;
        return value;
      }
