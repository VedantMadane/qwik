import { isServer, withLocale } from '@qwik.dev/core';
...
export const resolveHead = (
  ...
  const getData = ((loaderOrAction: LoaderInternal | ActionInternal) => {
    ...
    // Check AsyncSignals (loaders) — .value may throw a promise on the client
    const signal = loaderState?.[id];
    if (signal) {
      if (isServer && signal.loading) {
        throw new Error(
          `Loaders returning a promise can not be used for the head function. (${(signal as any).$computeQrl$?.dev?.file || (signal as any).$computeQrl$?.getHash()} || 'unknown')`
        );
      }
      return signal.value;
    }
